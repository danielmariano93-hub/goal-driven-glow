...
- **Onda 2 — concluída** (01/08/2026): `cycleFor/openCycleOf/previousCycleOf` + `formingStatement` em `cardExposure.ts` (espelhado), `todaySP()` como âncora única de data, função SQL `card_cycle_for`, backfill de `period_start/period_end/closing_date`, absorção de parcelas **apenas** por vínculo item↔parcela em fatura fechada/paga (com gatilhos de sincronização) e recálculo de `competence_date` das compras fora de fatura. Resultado real: fatura do Daniel 26/06–25/07, vencimento 03/08, 41 parcelas absorvidas por item, `v_card_double_counting` com resíduo R$ 0,00.
- **Onda 3 — concluída** (01/08/2026): `finance_contract.v1`. `facts`, `spendingRhythm`, `dailyAverage`, `cardExposure` e `metrics` passaram a ser sincronizados para `_shared/finance-core`; `_shared/engine/facts.ts` virou re-export puro e `_shared/engine/metrics.ts` agora deriva o snapshot do agente de `computeFinancialSnapshot` (dívida de cartão oficial, estimativa rotulada, parcelas futuras separadas, ritmo típico). MCP `financial_position` passou a usar `computeCardExposure`. Deploy: `agent-chat`, `agent-run`, `insights-generate`, `mcp`, `artifact-render`, `pulse-compute`, `assistant-ingest-document`, `assistant-review-actions`. 865 testes verdes.
- **Ondas 4 e 5 — pendentes** (contrato `_shared/http` nas 17 funções restantes, `partial_success`, crons, árvore `queryKeys.finance.*`, flags de gamificação/metas conjuntas e remoção da V1 de split).
