...
| Tempo | `_shared/analytics/periodResolver.ts` (`resolvePeriodPt`, `resolvePeriodRolesPt`) cobre "mês passado", "últimos N dias/meses", "mesmo período do mês passado". **Não tem aspecto habitual/typical**, e "últimos N meses" termina hoje (`complete: false`, linhas 108-118), não em meses fechados. `DeterministicAnswers.ts:286-297` ignora o resolver e usa janela fixa de 30 dias. |
...
- Migration nova (necessária): tabela `pending_write_workflows` (`user_id`, `conversation_id`, `channel`, `workflow_type`, `status`, `slots jsonb`, `missing_slots text[]`, `last_prompted_slot`, `pending_draft_id`, `pending_confirmation_id`, `source_message_id`, `created_at`, `updated_at`, `expires_at`), com RLS, GRANTs (`authenticated` self-view, `service_role` completo) e índice único parcial de um workflow ativo por conversa. Justificativa: `agent_sessions.state` expira em 30 min e o draft em 15 min, então workflow em cima deles perderia estado no meio da conversa.
- `core/WriteWorkflowManager.ts` (novo): leitura/escrita transacional do workflow via RPC, TTL de 30 min renovado a cada turno. Roda **depois** do `ConfirmationFastPath` e **antes** de qualquer roteamento normal (incluindo antes da heurística de `AgentCore.ts:1862-1889`, que passa a ser fallback só sem workflow ativo).
