...
import { buildHomeGuidancePresentation, projectionCanBeDetailed } from "@/lib/nino/homeGuidance";
...
export function NinoGuidanceCard({ diagnosis, projection, loading, error, retrying, onRetry, projectionAvailability = "available" }: Props) {
  ...
  if (error) return <section aria-label="Orientação do Nino" className="min-h-[156px] rounded-2xl border border-border bg-card p-4"><NinoErrorBlock error={error} onRetry={onRetry} retrying={retrying} /></section>;
  if (!item || !presentation) return <section aria-label="Orientação do Nino" className="relative min-h-[156px] overflow-hidden rounded-2xl border border-border bg-card p-5"><span className="absolute inset-y-2 left-0 w-[3px] rounded-r-full bg-primary" aria-hidden="true" /><p className="text-xs font-bold text-primary">Orientação do Nino</p><h2 className="mt-2 text-base font-bold text-foreground">Ainda estou formando uma leitura segura</h2><p className="mt-1 text-sm leading-relaxed text-muted-foreground">Com mais movimentações, consigo explicar o que mudou sem tirar conclusões apressadas.</p></section>;

  const detailProjection = projectionCanBeDetailed(projection, projectionAvailability);
  const confidenceNote = detailProjection ? confidenceCopy[detailProjection.confidence] : null;
  return (
    <section aria-label="Orientação do Nino" className="relative overflow-hidden rounded-2xl border border-border bg-card p-5 animate-fade-in">
      <span className={`absolute inset-y-2 left-0 w-[3px] rounded-r-full ${accent}`} aria-hidden="true" />
      <div className="flex items-center gap-2"><Icon className={critical ? "text-destructive" : attention ? "text-warning" : "text-primary"} weight="duotone" /><p className="text-xs font-bold text-foreground">Orientação do Nino</p><span className="sr-only">{critical ? "Crítico" : attention ? "Atenção" : "Informativo"}</span></div>
      <h2 className="mt-3 text-lg font-bold leading-snug text-foreground">{presentation.title}</h2>
      {presentation.supportingText ? <p className="mt-2 text-sm leading-relaxed text-muted-foreground">{presentation.supportingText}</p> : null}
      {action ? <div className="mt-4 border-t border-border pt-4"><p className="text-xs font-semibold text-muted-foreground">Melhor ação agora</p>{action.explanation ? <p className="mt-1 text-xs leading-relaxed text-muted-foreground">{action.explanation}</p> : null}<Button asChild className="mt-3 min-h-12 w-full rounded-xl sm:w-auto"><Link to={action.route} onClick={() => feedback.mutate({ situationId: item.id, feedback: "acted", surface: "home" })}>{action.title}<ArrowRight weight="bold" /></Link></Button></div> : null}
      <Collapsible open={open} onOpenChange={setOpen} className="mt-3">
        ...
        <CollapsibleContent className="pt-2 text-xs leading-relaxed text-muted-foreground">
          {item.cause_summary ? <p>{item.cause_summary}</p> : null}
          {diagnosis.counterpoint ? <p className="mt-2"><strong className="text-foreground">Também vale saber:</strong> {diagnosis.counterpoint.one_line_summary || diagnosis.counterpoint.headline}</p> : null}
          {item.consequence_summary ? <p className="mt-2">{item.consequence_summary}</p> : null}
          {item.forecast_summary && projectionAvailability === "available" ? <p className="mt-2">{item.forecast_summary}</p> : null}
          {diagnosis.evidenceSummary ? <p className="mt-2">{diagnosis.evidenceSummary}</p> : null}
          ...
          {detailProjection ? <ProjectionRows projection={detailProjection} /> : null}
