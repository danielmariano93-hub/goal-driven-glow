...
export function AssistantTipCard({ diagnosis, loading, error, retrying, onRetry }: InsightProps) {
  ...
  if (loading) return <section aria-label="Leitura do Nino" className="min-h-[124px] rounded-[20px] border border-primary/15 bg-accent p-4" aria-busy="true"><div className="h-3 w-24 animate-pulse rounded bg-primary/10" /><div className="mt-3 h-5 w-3/4 animate-pulse rounded bg-primary/10" /><div className="mt-2 h-3 w-full animate-pulse rounded bg-primary/10" /></section>;
  if (error) return <section aria-label="Leitura do Nino"><NinoErrorBlock error={error} onRetry={onRetry} retrying={retrying} /></section>;
  if (!item) return <section aria-label="Leitura do Nino" className="rounded-[20px] border border-primary/15 bg-accent p-4"><p className="text-[10px] font-bold uppercase text-primary">Leitura do Nino</p><h2 className="mt-2 text-[15px] font-bold text-foreground">Ainda estou formando uma leitura segura</h2><p className="mt-1 text-[12px] leading-relaxed text-muted-foreground">Com mais movimentações, consigo explicar o que mudou sem tirar conclusões apressadas.</p></section>;

  return (
    <section aria-label="Leitura do Nino" className="rounded-[20px] border border-primary/15 bg-accent p-4">
      <div className="flex items-center gap-2"><span className="grid h-8 w-8 place-items-center rounded-md bg-primary/10 text-primary"><Lightbulb className="h-4 w-4" /></span><div><p className="text-[10px] font-bold uppercase text-primary">Leitura do Nino</p><p className="text-[10px] text-muted-foreground">Confiança {Math.round(diagnosis.diagnosisConfidence * 100)}%{dateLabel(diagnosis.asOf) ? ` · ${dateLabel(diagnosis.asOf)}` : ""}</p></div></div>
...
export function BestActionCard({ diagnosis, loading, onRefresh, refreshing }: { diagnosis: HomeDiagnosisView | null; loading?: boolean; onRefresh?: () => void; refreshing?: boolean }) {
  ...
  return <section aria-label="Melhor ação agora" className="rounded-[20px] bg-gradient-brand-dark p-4 text-primary-foreground"><p className="text-[10px] font-bold uppercase text-primary-foreground/65">Melhor ação agora</p>{loading ? <div className="mt-3 flex items-center gap-2 text-[12px] text-primary-foreground/70"><Loader2 className="h-4 w-4 animate-spin" /> Buscando o próximo passo</div> : action && situation && route ? <div className="mt-2 grid gap-3 sm:grid-cols-[1fr_auto] sm:items-center"><div className="min-w-0"><h2 className="text-[15px] font-bold leading-snug">{action.title}</h2>{action.explanation ? <p className="mt-1 text-[12px] leading-relaxed text-primary-foreground/70">{action.explanation}</p> : null}{typeof action.estimated_impact === "number" ? <p className="mt-1 text-[11px] font-semibold tabular-nums text-primary-foreground/85">Impacto estimado: {formatBRL(action.estimated_impact)}</p> : null}</div><Button asChild variant="secondary" className="min-h-11 rounded-full"><Link to={route} onClick={() => feedback.mutate({ situationId: situation.id, feedback: "acted", surface: "home" })}>{action.title}<ArrowRight /></Link></Button></div> : <div className="mt-2 flex items-center justify-between gap-4"><div><h2 className="text-[15px] font-bold">Continuar acompanhando</h2><p className="mt-1 text-[12px] text-primary-foreground/70">Não há uma ação confiável recomendada agora.</p></div>{onRefresh ? <Button type="button" variant="ghost" size="icon" onClick={onRefresh} disabled={refreshing} aria-label="Atualizar leitura do Nino" className="h-11 w-11 shrink-0 rounded-full text-primary-foreground hover:bg-primary-foreground/10 hover:text-primary-foreground">{refreshing ? <Loader2 className="animate-spin" /> : <RefreshCw />}</Button> : null}</div>}</section>;
}
