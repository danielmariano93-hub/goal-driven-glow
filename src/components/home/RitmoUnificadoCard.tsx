...
export function RitmoUnificadoCard({ rhythm, projection, loading, partial, error, onRetry }: Props) {
  ...
  return (
    <section aria-label="Ritmo de gastos" className="overflow-hidden rounded-[24px] border border-border bg-card shadow-hero animate-fade-in">
      <div className="flex items-start justify-between gap-3 px-5 pt-5"><div className="min-w-0"><p className="text-xs font-semibold text-primary">Seu comportamento</p><h2 className="mt-1 font-display text-lg font-bold leading-6 text-foreground">Seu ritmo de gastos</h2>{loading ? <div className="mt-2 h-8 w-36 animate-pulse rounded bg-secondary" /> : <p className="mt-3 font-display text-[28px] font-bold leading-[34px] tabular-nums text-foreground">{current ? formatBRL(current.average) : "—"}<span className="font-interface text-xs font-semibold text-muted-foreground">/dia</span></p>}<div className="mt-2"><Comparison trend={rhythm?.averageTrend ?? "stable"} deltaPct={hasPrevious ? rhythm?.averageDeltaPct ?? null : null} deltaAmount={averageDelta} /></div>{typicalPace > 0 ? <p className="mt-2 text-xs text-muted-foreground">Seu padrão habitual: <strong className="text-foreground">{formatBRL(typicalPace)}/dia</strong></p> : <p className="mt-2 text-xs text-muted-foreground">Ainda estamos aprendendo seu padrão habitual.</p>}{partial ? <p className="mt-2 text-xs text-muted-foreground">Comparação parcial enquanto atualizamos algumas fontes.</p> : null}</div><Button type="button" variant="ghost" size="icon" onClick={() => setMethodOpen(true)} aria-label="Como calculamos o ritmo" className="h-11 w-11 shrink-0 rounded-full text-muted-foreground"><Info /></Button></div>
      <p className="sr-only">{summary}</p>
       <div className={hasData || loading ? "mt-3 min-h-[178px] border-t border-border px-1 pt-2" : "mt-2"}>{loading ? <div className="mx-4 h-[168px] animate-pulse rounded-2xl bg-secondary" /> : !hasData ? <div className="grid min-h-[112px] place-items-center px-6 pb-4 text-center"><div><p className="text-sm text-muted-foreground">Ainda não há gastos neste período.</p><Button asChild variant="ghost" className="mt-2 min-h-11 text-primary"><Link to="/app/lancamentos">Anotar movimentação</Link></Button></div></div> : <ResponsiveContainer width="100%" height={174}><ComposedChart data={data} margin={{ top: 10, right: 12, left: -20, bottom: 0 }}><defs><linearGradient id="rhythmFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="hsl(var(--primary))" stopOpacity={0.06} /><stop offset="100%" stopColor="hsl(var(--primary))" stopOpacity={0} /></linearGradient></defs><CartesianGrid stroke="hsl(var(--border))" vertical={false} horizontalValues={[0]} /><XAxis dataKey="day" tickFormatter={(value) => `D${value}`} tick={{ fontSize: 12, fill: "hsl(var(--muted-foreground))" }} tickLine={false} axisLine={false} minTickGap={24} /><YAxis hide domain={[0, "auto"]} /><ChartTooltip content={<ChartDetail />} cursor={{ stroke: "hsl(var(--border))" }} /><Area type="monotone" dataKey="currentAmount" stroke="none" fill="url(#rhythmFill)" /><Line type="monotone" dataKey="currentAmount" stroke="hsl(var(--primary))" strokeWidth={2.5} dot={false} activeDot={{ r: 4, strokeWidth: 4 }} /><Line type="monotone" dataKey="previousAmount" stroke="hsl(var(--muted-foreground))" strokeWidth={1.5} strokeDasharray="6 4" dot={false} activeDot={{ r: 4 }} connectNulls /></ComposedChart></ResponsiveContainer>}</div>
       {hasData ? <div className="flex items-center gap-4 px-5 pb-2 text-xs text-muted-foreground"><span className="inline-flex items-center gap-1.5"><i className="h-0.5 w-4 bg-primary" />Este período</span><span className="inline-flex items-center gap-1.5"><i className="h-0 w-4 border-t border-dashed border-muted-foreground" />Período anterior</span></div> : null}
       {hasData ? <div className="px-5 pb-4"><Button asChild variant="ghost" className="min-h-11 w-full justify-start px-0 text-primary"><Link to="/app/lancamentos">Ver rotina e categorias</Link></Button></div> : null}
