...
import { ArrowDownRight, ArrowUpRight, CaretDown, Info, Minus } from "@phosphor-icons/react";
...
function Comparison({ trend, deltaPct }: { trend: Trend; deltaPct: number | null }) {
  ...
  return <span className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-[10px] font-semibold tabular-nums ${higher ? "bg-brand-coral/10 text-brand-coral" : "bg-success/10 text-success"}`}><Icon weight="bold" /> {Math.abs(deltaPct).toFixed(1).replace(".", ",")}% {higher ? "acima" : "abaixo"}</span>;
}
...
export function RitmoUnificadoCard({ rhythm, projection, loading }: Props) {
  ...
  return (
    <section aria-label="Ritmo de gastos" className="overflow-hidden rounded-[20px] border border-primary/15 bg-card shadow-sm animate-fade-in">
      <div className="h-1 bg-gradient-to-r from-primary via-primary to-brand-coral" aria-hidden="true" />
      <div className="flex items-end justify-between gap-3 px-4 pt-4"><div className="min-w-0"><p className="text-[10px] font-bold uppercase text-primary">Ritmo de gastos</p>{loading ? <div className="mt-2 h-8 w-36 animate-pulse rounded bg-secondary" /> : <p className="mt-1 font-display text-[28px] font-extrabold leading-none tabular-nums text-foreground">{current ? formatBRL(current.average) : "—"}<span className="text-[12px] font-semibold text-muted-foreground">/dia</span></p>}<p className="mt-2 text-[11px] text-muted-foreground">Seu típico: <span className="font-semibold text-foreground">{projection ? `${formatBRL(projection.typicalDailyPace)}/dia` : "—"}</span></p></div><Comparison trend={rhythm?.averageTrend ?? "stable"} deltaPct={rhythm?.averageDeltaPct ?? null} /></div>
