...
export function RitmoUnificadoCard({ rhythm, projection, loading, partial, error, onRetry }: Props) {
  ...
  return (
    <section aria-label="Ritmo de gastos" className="overflow-hidden rounded-2xl border border-border bg-card animate-fade-in">
      <div className="flex items-start justify-between gap-3 px-4 pt-4"><div className="min-w-0"><p className="text-xs font-bold text-primary">Ritmo de gastos</p>{loading ? <div className="mt-2 h-8 w-36 animate-pulse rounded bg-secondary" /> : <p className="mt-2 font-display text-[28px] font-extrabold leading-none tabular-nums text-foreground">{current ? formatBRL(current.average) : "—"}<span className="text-xs font-semibold text-muted-foreground">/dia</span></p>}<div className="mt-2"><Comparison trend={rhythm?.averageTrend ?? "stable"} deltaPct={hasPrevious ? rhythm?.averageDeltaPct ?? null : null} deltaAmount={averageDelta} /></div>{projection?.typicalDailyPace ? <p className="mt-2 text-xs text-muted-foreground">Ritmo típico: <strong className="text-foreground">{formatBRL(projection.typicalDailyPace)}/dia</strong></p> : null}{partial ? <p className="mt-2 text-xs text-muted-foreground">Comparação parcial enquanto atualizamos algumas fontes.</p> : null}</div><Button type="button" variant="ghost" size="icon" onClick={() => setMethodOpen(true)} aria-label="Como calculamos o ritmo" className="h-11 w-11 shrink-0 rounded-full text-muted-foreground"><Info /></Button></div>
      <p className="sr-only">{summary}</p>
       <div className={hasData || loading ? "mt-3 min-h-[154px] border-t border-border px-1 pt-2" : "mt-2"}>{loading ? <div className="mx-3 h-[148px] animate-pulse rounded-xl bg-secondary" /> : !hasData ? <div className="grid min-h-[72px] place-items-center px-6 pb-4 text-center text-xs text-muted-foreground">Assim que houver gastos, seu ritmo aparece aqui.</div> : <ResponsiveContainer width="100%" height={154}><LineChart data={data} margin={{ top: 10, right: 12, left: 4, bottom: 0 }}><CartesianGrid stroke="hsl(var(--border))" vertical={false} /><XAxis dataKey="day" tickFormatter={(value) => `D${value}`} tick={{ fontSize: 10, fill: "hsl(var(--muted-foreground))" }} tickLine={false} axisLine={false} minTickGap={24} /><YAxis hide /><ChartTooltip content={<ChartDetail />} /><Line type="linear" dataKey="currentAmount" stroke="hsl(var(--primary))" strokeWidth={2.5} dot={false} activeDot={{ r: 4 }} />{hasPrevious ? <Line type="linear" dataKey="previousAmount" stroke="hsl(var(--muted-foreground))" strokeWidth={1.5} strokeDasharray="5 4" dot={false} /> : null}</LineChart></ResponsiveContainer>}</div>
       {hasData ? <div className="flex items-center gap-4 px-4 pb-4 text-[10px] text-muted-foreground"><span className="inline-flex items-center gap-1.5"><i className="h-0.5 w-4 bg-primary" />Atual</span>{hasPrevious ? <span className="inline-flex items-center gap-1.5"><i className="h-0 w-4 border-t border-dashed border-muted-foreground" />Anterior</span> : null}</div> : null}
