...
/**
 * Exact instant of a live transaction when the product genuinely knows it.
 * `local_occurred_at` wins. For manual/agent writes made for the current local
 * day, `created_at` is a safe fallback. Imported statement rows deliberately do
 * not use import time because that is not the bank posting instant.
 */
...
/**
 * Returns new snapshot objects; never mutates persisted bank evidence.
 * Unknown anchor/transaction times keep the conservative legacy behavior.
 */
