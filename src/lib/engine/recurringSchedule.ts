...
/** Próxima ocorrência (ou null quando a regra já terminou). */
export function nextOccurrenceFor(rule: RecurringRule, from: string): string | null {
  return nextOccurrences(rule, from, 1)[0] ?? null;
}
