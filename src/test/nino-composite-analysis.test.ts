...
function category(over: Record<string, unknown> = {}) {
  return {
    ...
    historical: { trend: "improved", current: 900, previous: 1200, delta: -300, confidence: "high" },
...
function assessment(categories: any[] = [category()]) {
  return {
    ...
    period: { current: { from: "2026-08-01", to: "2026-08-20" }, comparison: { from: "2026-07-01", to: "2026-07-20" } },
    ...
    aggregate: {
      ...
      vs_target: 100, vs_target_pct: 12.5, vs_previous: -300, vs_previous_pct: -25,
    },
    conclusions: {
...
describe("InterpretationResolver", () => {
  it("separa meta estourada de piora comportamental", () => {
    ...
    expect(r.conclusion).toMatch(/menos que no per[íi]odo anterior/i);
  });
  ...
  it("meta cumprida com gasto maior não vira melhora", () => {
    const r = resolveInterpretation(assessment([category({
      ...
      historical: { trend: "worsened", current: 900, previous: 600, delta: 300, confidence: "high" },
  ...
  it("sem histórico não afirma melhora", () => {
    const r = resolveInterpretation(assessment([category({
      historical: { trend: "insufficient_data", current: 900, previous: 0, delta: 900, confidence: "insufficient" },
    })]));
