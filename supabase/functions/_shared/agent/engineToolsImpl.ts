...
export function merchant_profile(
  ...
  return guard(async () => {
    ...
    const f = env.facts;
    const categorySuffix = categoryName ? ` em ${categoryName}` : "";
    const headline = f.found
      ? `${f.label}${categorySuffix}: ${brl(f.net_total)} em ${f.count} compra(s), ticket médio ${brl(f.avg_ticket)}.`
      : `Não encontrei lançamentos de “${f.query}”${categorySuffix} nessa janela.`;
    return {
      ...withAnswerFormat(env, headline, f.delta_abs),
      period,
      filters: {
        merchant: String(args?.query ?? "").trim(),
        category: categoryName,
      },
      total_metric: f.net_total,
      transactions_count: f.count,
    };
  });
}
