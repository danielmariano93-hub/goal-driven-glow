...
/** Identidade canônica de webhook.
...
export function compareWebhookIdentity(
  hook: { url?: string; events?: string[]; customHeaders?: Array<{ name?: string; value?: string }> } | null | undefined,
  expectedBaseUrl: string,
  secret: string,
): WebhookIdentity {
  const actual = canonicalRoute(hook?.url ?? "");
  const expected = canonicalRoute(expectedBaseUrl);
  const routeValid = Boolean(actual && expected && actual.origin === expected.origin && actual.path === expected.path);
  ...
  const headerOk = Boolean(
    hook?.customHeaders?.some((h) =>
      (h?.name ?? "").toLowerCase() === "x-webhook-secret" && Boolean(h?.value) && (!secret || h?.value === secret)
    ),
  );
  ...
  return {
    ...
    eventsValid: REQUIRED_EVENTS.every((e) => events.includes(e)),
  };
}
