// Centralized AI usage ledger. Best-effort only: telemetry must never
...
export function estimateAiCostUsd(model: string | null | undefined, inputTokens = 0, outputTokens = 0): number {
  ...
  const [per1KIn, per1KOut] =
      /gpt-oss-20b/.test(m) ? [0.000075, 0.00030]
    : /gpt-oss-120b/.test(m) ? [0.00015, 0.00060]
    : /qwen3\.6-27b/.test(m) ? [0.00060, 0.00300]
    : /qwen3\.8-27b/.test(m) ? [0.00080, 0.00400]
    ...
    : /transcribe|whisper|audio/.test(m) ? [0.0006, 0.0000]
    : [0.0010, 0.0040];
...
export async function recordAiUsage(sb: SupabaseClient, event: AiUsageEvent): Promise<void> {
  try {
    ...
    const provider = String(event.provider ?? event.metadata?.provider ?? "unknown");
