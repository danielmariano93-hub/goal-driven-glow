...
const REPORT_DAILY_CHART_VERSION = "financial_report.daily_combo.v1";
...
function trailingAverage(values: number[], window = 7): number[] {
...
function monthlyChartCaption(report: IntelligentReport, link: string | null): string {
  ...
  const lines = [
    ...
    `• Total: ${brl(totals.expense)} em ${totals.daysWithExpense} dias com gasto.`,
  ];
  if (peak && peak.expense > 0) {
    lines.push(`• Maior dia: ${brl(peak.expense)} em ${peak.label}.`);
  }
...
function buildMonthlyDailyChart(report: IntelligentReport, fallbackText: string) {
  ...
  return {
    ...
    a11y_summary: `Gastos diários de ${report.period.start} a ${report.period.end}. Total de ${brl(report.payload.totals.expense)}.`,
    chart: {
      type: "bar",
      title: "Gasto diário e média móvel de 7 dias",
      x_labels: report.payload.series.map((point) => point.label),
      series: [
        { name: "Gasto diário", data: daily, color: "#6D3BFF", render_as: "bar" },
        { name: "Média de 7 dias", data: trailingAverage(daily, 7), color: "#FF9F1C", render_as: "line" },
      ],
      units: "BRL",
      y_format: "currency",
    },
    provenance: {
      ...
      confidence: report.dataQualityStatus === "ok" ? "high" : "medium",
