...
const BodySchema=z.object({operation:z.enum(["classify","classify_batch","learn","review_status","process_queue","process_queue_global","backfill"]),input:InputSchema.optional(),inputs:z.array(InputSchema).optional(),transaction_id:z.string().optional(),category_id:z.string().optional(),limit:z.number().int().min(1).max(500).optional()});
