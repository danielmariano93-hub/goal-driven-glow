...
Deno.serve(async (req) => {
  const h = httpContext("user-ai-preferences", req);
  if (req.method === "OPTIONS") return new Response("ok", { headers: corsHeaders });
  if (req.method !== "GET" && req.method !== "POST") return h.fail("method_not_allowed", 405);
  ...
  if (req.method === "GET") {
    return h.ok({ preferences: await loadPreferences(sb, user_id), defaults: DEFAULT_PREFS });
  }
