// Outbound worker. Uses claim_outbound_batch (SKIP LOCKED) to prevent
// duplicate sends across concurrent workers. Simulator-channel rows are
// never sent to WAHA — they're marked 'sent' at insert time.
...
async function requireAdmin(req: Request) {
  ...
  if (!userRes.user) return { ok: false, status: 401 };
...
Deno.serve(async (req) => {
  ...
  for (const m of rows) {
    try {
      ...
      await sb.from("outbound_messages").update({ provider_message_id }).eq("id", m.id);
