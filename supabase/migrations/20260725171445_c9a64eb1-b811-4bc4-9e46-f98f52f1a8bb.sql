...
BEGIN
  ...
  INSERT INTO public.whatsapp_links(user_id, phone_e164, phone_hash, phone_masked, status) VALUES
  ...
  BEGIN PERFORM public.shared_goal_add_contribution(v_goal, 10, current_date, NULL, NULL);
    v_result := v_result || jsonb_build_object('outsider_blocked', false);
  EXCEPTION WHEN OTHERS THEN v_result := v_result || jsonb_build_object('outsider_blocked', true);
  END;
  PERFORM set_config('request.jwt.claim.sub', v_pending::text, true);
  BEGIN PERFORM public.shared_goal_add_contribution(v_goal, 10, current_date, NULL, NULL);
    v_result := v_result || jsonb_build_object('pending_blocked', false);
  EXCEPTION WHEN OTHERS THEN v_result := v_result || jsonb_build_object('pending_blocked', true);
  END;
  PERFORM public.shared_goal_decline_invite(v_goal);
  v_result := v_result || jsonb_build_object(
    'pending_declined', (SELECT status FROM public.shared_goal_invites WHERE goal_id=v_goal AND phone_e164=v_phone_pending) = 'declined'
  );
  PERFORM set_config('request.jwt.claim.sub', v_owner::text, true);
  BEGIN PERFORM public.shared_goal_leave(v_goal);
    v_result := v_result || jsonb_build_object('owner_cannot_leave', false);
  EXCEPTION WHEN OTHERS THEN v_result := v_result || jsonb_build_object('owner_cannot_leave', true);
  END;
  PERFORM set_config('request.jwt.claim.sub', v_outsider::text, true);
  BEGIN PERFORM public.shared_goal_update(v_goal, 'Hack', NULL, NULL);
    v_result := v_result || jsonb_build_object('outsider_update_blocked', false);
  EXCEPTION WHEN OTHERS THEN v_result := v_result || jsonb_build_object('outsider_update_blocked', true);
  END;
  PERFORM set_config('request.jwt.claim.sub', v_owner::text, true);
