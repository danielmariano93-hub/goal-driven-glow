CREATE OR REPLACE FUNCTION public.update_credit_card_statement_item(
  p_item_id uuid,p_description text,p_category_id uuid DEFAULT NULL,p_amount numeric DEFAULT NULL,p_occurred_at date DEFAULT NULL,p_item_kind text DEFAULT NULL
) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE v_user uuid:=auth.uid(); v_item public.credit_card_statement_items%ROWTYPE; v_amount numeric(14,2); v_kind text; v_activity numeric(14,2); v_statement public.credit_card_statements%ROWTYPE; v_reconciled numeric(14,2);
BEGIN
  ...
  IF v_statement.status NOT IN ('draft','needs_review') OR v_statement.paid_amount>0 THEN RAISE EXCEPTION 'statement_economic_fields_locked' USING ERRCODE='55000'; END IF;
  ...
  IF v_item.legacy_transaction_id IS NOT NULL THEN UPDATE public.transactions SET description=trim(p_description),category_id=p_category_id,category_source='user',amount=abs(v_amount),occurred_at=coalesce(p_occurred_at,occurred_at),type=CASE WHEN v_kind='refund' THEN 'income'::public.transaction_type ELSE 'expense'::public.transaction_type END,updated_at=now() WHERE id=v_item.legacy_transaction_id AND user_id=v_user; END IF;
  ...
  UPDATE public.credit_card_statements SET reconciled_total=v_reconciled,status='needs_review',updated_at=now() WHERE id=v_item.statement_id;
