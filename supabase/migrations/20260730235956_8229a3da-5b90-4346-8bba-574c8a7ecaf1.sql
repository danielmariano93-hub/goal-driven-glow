-- Reconcilia a fatura pelo ciclo completo:
-- saldo anterior + compras/encargos - créditos/estornos - pagamentos = total oficial.
-- O saldo anterior é metadado auditável, nunca um novo lançamento financeiro.
ALTER TABLE public.document_imports
  ADD COLUMN IF NOT EXISTS invoice_previous_balance numeric(14,2);

ALTER TABLE public.credit_card_statements
  ADD COLUMN IF NOT EXISTS opening_balance numeric(14,2) NOT NULL DEFAULT 0;

COMMENT ON COLUMN public.document_imports.invoice_previous_balance IS
  'Saldo explicitamente trazido da fatura anterior. Não gera transaction.';
COMMENT ON COLUMN public.credit_card_statements.opening_balance IS
  'Saldo trazido do ciclo anterior, usado apenas na conciliação da fatura.';
