-- ============================================================================
-- ADMIN CONTROL CENTER — CONTRACT REPAIR
-- Additive/corrective migration. Do not edit previously applied migrations.
-- ============================================================================
...
-- --------------------------------------------------------------------------
-- 2. Remove ambiguous old overload and recreate one canonical health RPC
-- --------------------------------------------------------------------------
...
AS $$
DECLARE
  v_cutoff timestamptz := now() - make_interval(hours => greatest(_hours, 1));
BEGIN
      ...
      SELECT coalesce(
        jsonb_agg(
          jsonb_build_object(
            ...
            'last_ok', j.last_ok,
            'processed', coalesce(j.processed, 0),
            'failed', coalesce(j.failed, 0),
      ...
      SELECT jsonb_build_object(
        ...
        'success_rate',
          CASE WHEN count(*) = 0 THEN null
               ELSE round((count(*) filter (where status::text = 'done'))::numeric / count(*) * 100, 1)
          END,
        'p50_ms', percentile_cont(0.50) within group (order by latency_ms)
  ...
  RETURN jsonb_build_object(
    ...
    (
      SELECT jsonb_build_object(
        ...
        'confirmation_rate',
          CASE
            WHEN count(*) filter (where status <> 'canceled') = 0 THEN null
            ELSE round(
              (count(*) filter (where status = 'confirmed'))::numeric /
              (count(*) filter (where status <> 'canceled')) * 100,
              1
            )
          END,
        'failure_rate',
          ...
          END,
        'p50_ms',
          percentile_cont(0.50) within group (order by extraction_ms)
            filter (where extraction_ms is not null),
        'p95_ms',
          percentile_cont(0.95) within group (order by extraction_ms)
            filter (where extraction_ms is not null),
        'backlog',
          count(*) filter (
            where status not in ('confirmed','partially_confirmed','partial','failed','canceled')
          )::int
      )
  ...
  RETURN jsonb_build_object(
    ...
    (
      SELECT jsonb_build_object(
        'attempts', count(*)::int,
        'sent', count(*) filter (
          where status::text in ('sent','delivered','read')
        ...
        'backlog', count(*) filter (
          where status::text not in ('sent','delivered','read','failed','dead')
        )::int
      )
      FROM public.outbound_messages
      WHERE created_at >= v_cutoff
...
-- --------------------------------------------------------------------------
-- 5. Growth funnel without nested aggregates
-- --------------------------------------------------------------------------
    ...
    (
      WITH normalized AS (
        SELECT
          ...
          CASE
            ...
            ) AND outcome IN ('success','partial') THEN 'value_delivered'
            ELSE NULL
          END AS step
      ...
      grouped AS (
        SELECT
          feature,
          step,
          count(distinct pseudo_id)::int as users,
          count(*)::int as events
        FROM normalized
...
-- --------------------------------------------------------------------------
-- 6. Client identity RPCs using the real audit table contract
-- --------------------------------------------------------------------------
          ...
          jsonb_build_object(
            ...
            'display_name',
              case
                ...
                else split_part(pr.display_name, ' ', 1) || ' ***'
              end,
            'email',
              case
                when au.email is null or position('@' in au.email)=0 then null
...
-- --------------------------------------------------------------------------
-- 7. Audit list using the actual platform_admin_audit columns
-- --------------------------------------------------------------------------
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'instrumentation_started_at', (
      SELECT min(created_at) FROM public.platform_admin_audit
    ),
...
-- --------------------------------------------------------------------------
-- 8. Grants: authenticated/service role only
-- --------------------------------------------------------------------------
DO $$
DECLARE r record;
BEGIN
