-- Unifica a confirmação de importação: mesma implementação para app (auth.uid())
-- e para o assessor via service role (p_user_id). O usuário autenticado sempre
-- prevalece, então o parâmetro não pode ser usado para spoof.
DROP FUNCTION IF EXISTS public.confirm_document_import(uuid, uuid[]);
...
AS $function$
DECLARE
  v_user uuid := coalesce(auth.uid(), CASE WHEN auth.uid() IS NULL THEN p_user_id ELSE NULL END);
...
BEGIN
  ...
  v_is_card := coalesce(v_doc.document_kind, '') = 'invoice';

  -- Guardrail de saldo vale só para extrato bancário: fatura não tem saldo de caixa.
  IF NOT v_is_card
     AND v_doc.statement_closing_balance IS NOT NULL
     AND v_doc.statement_opening_balance IS NOT NULL THEN
    SELECT
  ...
  LOOP
    ...
    -- Fatura: o item é do cartão. Nunca movimenta conta corrente.
    IF v_is_card AND coalesce(v_item.movement_kind, 'transaction') <> 'card_payment' THEN
      v_item.account_id := NULL;
      v_item.payment_method := 'credit_card';
      IF v_item.credit_card_id IS NULL THEN
        v_item.credit_card_id := v_doc.source_credit_card_id;
      END IF;
    END IF;
    ...
    v_src := 'document:'||p_document_id::text||':'||v_item.idx::text;

    -- Idempotência: se o item já virou transação numa tentativa anterior, reaproveita.
    SELECT id INTO v_new_tx_id FROM public.transactions
      WHERE user_id = v_user AND import_source_id = v_src LIMIT 1;
    IF v_new_tx_id IS NOT NULL THEN
      UPDATE public.extracted_items
         SET transaction_id = v_new_tx_id, status = 'confirmed'
       WHERE id = v_item.id;
      v_skipped := v_skipped || jsonb_build_object('item_id', v_item.id, 'reason', 'already_imported', 'transaction_id', v_new_tx_id);
      CONTINUE;
    END IF;
    ...
    BEGIN
      INSERT INTO public.transactions(
        ...
        v_item.amount,
        CASE WHEN v_item.payment_method = 'credit_card'
             THEN coalesce(v_item.purchase_date, v_item.occurred_at)
             ELSE v_item.occurred_at END,
        v_item.description,
...
GRANT EXECUTE ON FUNCTION public.confirm_document_import(uuid, uuid[], uuid) TO authenticated, service_role;

-- Acelera o candidato de deduplicação (mesma data/valor do usuário).
CREATE INDEX IF NOT EXISTS idx_transactions_user_date_amount
  ON public.transactions (user_id, occurred_at, amount);