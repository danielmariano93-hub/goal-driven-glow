-- Entrega corretiva única: fecha contratos de doação e comunicação emocional.
...
-- Compatibilidade dos desafios criados antes do catálogo novo.
...
-- A função antiga esperava status/colunas de uma versão de tabela que nunca
-- existiu neste projeto (`active`, `completed_at`, `last_progress_at`).
...
-- O motor de padrões de dia da semana existia, mas não possuía agendamento de
-- produção. Este cron executa detecção + fila + entrega com dry_run desligado.
DO $$
DECLARE v_jobid BIGINT;
BEGIN
  SELECT jobid INTO v_jobid FROM cron.job WHERE jobname = 'anticipation-hourly' LIMIT 1;
  ...
  PERFORM cron.schedule(
    ...
    $cron$
      SELECT net.http_post(
        url := 'https://wesjjdjmlnfjihkkgzfp.supabase.co/functions/v1/anticipation-tick',
