-- 1) Invariante de ledger: um item nunca pertence a conta e cartão ao mesmo tempo.
ALTER TABLE public.extracted_items
  DROP CONSTRAINT IF EXISTS extracted_items_single_ledger;
ALTER TABLE public.extracted_items
  ADD CONSTRAINT extracted_items_single_ledger
  CHECK (NOT (account_id IS NOT NULL AND credit_card_id IS NOT NULL)) NOT VALID;

ALTER TABLE public.transactions
  DROP CONSTRAINT IF EXISTS transactions_single_ledger;
ALTER TABLE public.transactions
  ADD CONSTRAINT transactions_single_ledger
  CHECK (NOT (account_id IS NOT NULL AND credit_card_id IS NOT NULL)) NOT VALID;

-- 2) Idempotência forte por item de documento.
CREATE UNIQUE INDEX IF NOT EXISTS transactions_import_source_uniq
  ON public.transactions (user_id, import_source_id)
  WHERE import_source_id IS NOT NULL;

-- 3) Confirmação consciente do tipo de documento.
  ...
  LOOP
    ...
    BEGIN
      INSERT INTO public.transactions(
        ...
        v_item.amount,
        -- Cartão reconhece na data da compra (competência).
