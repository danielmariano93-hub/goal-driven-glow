-- Materialização retroativa da Onda 1 (Meu Nino Consolidado).
-- Aplicada previamente em produção; este arquivo torna a migration idempotente e reproduzível.
-- Rollback operacional: as feature flags use_wave1_bill_payment / use_v2_artifact_normalizer permitem
-- desligar o comportamento novo; nenhum dado é apagado por este arquivo.

-- 1) Tabela snapshot pré-onda (append-only)
CREATE TABLE IF NOT EXISTS public.wave1_pre_snapshot (
  ...
  bill_payments integer NOT NULL
);
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='wave1_pre_snapshot' AND policyname='wave1_pre_snapshot admin read') THEN
    CREATE POLICY "wave1_pre_snapshot admin read" ON public.wave1_pre_snapshot
      FOR SELECT TO authenticated USING (has_role(auth.uid(),'admin'::app_role));
  END IF;
END $$;

-- 2) Feature flags de rollback operacional
...
-- 3) Trigger de preenchimento de competência (rede de segurança)
...
CREATE TRIGGER trg_transactions_fill_competence_date BEFORE INSERT OR UPDATE OF credit_card_id, occurred_at ON public.transactions FOR EACH ROW EXECUTE FUNCTION transactions_fill_competence_date()
;

-- 4) Sweep de agent_runs órfãos
...
CREATE OR REPLACE FUNCTION public._exec_credit_card_bill_payment(c pending_confirmations)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
