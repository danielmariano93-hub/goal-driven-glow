...
BEGIN
  ...
  INSERT INTO public.whatsapp_links(user_id, phone_e164, phone_hash, phone_masked, status)
    VALUES
    (v_member, v_phone_member, encode(digest(v_phone_member,'sha256'),'hex'), '+55 11 9****-0001', 'active'),
    (v_pending, v_phone_pending, encode(digest(v_phone_pending,'sha256'),'hex'), '+55 11 9****-0002', 'active');
